...
<div class='header'>
	<?php echo "<h4>Core 1.2 feature coverage on ".PageGenerator::platformInfo($platform) ?>
</div>
		...
		<table id="features" class="table table-striped table-bordered table-hover responsive with-platform-selection">
			...
			<tbody>
				...
				try {
					...
					$deviceCount = DB::getCount("SELECT count(distinct(displayname)) from reports r join devicefeatures12 dp on dp.reportid = id $os_filter", $params);					
					if ($deviceCount > 0) {
						...
						$stmnt = DB::$connection->prepare(
							"SELECT r.displayname as device, " . implode(',', $columns) . " FROM devicefeatures12 df join deviceproperties dp on dp.reportid = df.reportid join reports r on r.id = df.reportid $os_filter group by device"
						);
