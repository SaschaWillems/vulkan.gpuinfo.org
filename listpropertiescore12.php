...
<div class='header'>
	<?php echo "<h4>Core 1.2 properties for ".PageGenerator::platformInfo($platform) ?>
</div>
		...
		<table id="properties" class="table table-striped table-bordered table-hover responsive with-platform-seelction">
			...
			<tbody>
				...
				try {
					...
					$count = DB::getCount("SELECT count(*) from reports r join deviceproperties12 dp on dp.reportid = id $os_filter", $params);
					if ($count > 0) {
						...
						$stmnt = DB::$connection->prepare(
							"SELECT r.displayname as device, " . implode(',', $columns) . " FROM deviceproperties12 dp join reports r on r.id = dp.reportid $os_filter group by device"
						);
						...
						$deviceCount = DB::getCount("SELECT count(distinct(displayname)) from reports r join deviceproperties12 dp on dp.reportid = id $os_filter", $params);
						...
						while ($row = $stmnt->fetch(PDO::FETCH_NUM, PDO::FETCH_ORI_NEXT)) {
							...
							$link = "<a href='displaycoreproperty.php?core=1.2&name=" . $row[0] . "&platform=$platform'>";
							echo "<tr>";
							echo "<td>" . $row[0] . "</a></td>";
							echo "<td class='text-center'>" . ($has_coverage ? 'Coverage' : 'Values') . "</td>";
							if ($has_coverage) {
								$coverageLink = "listdevicescoverage.php?core=1.2&coreproperty=" . $row[0] . "&platform=$platform";
