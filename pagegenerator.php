...
HTML;
    ...
    // Displays a human readable version of the currently selected filter
	...
	// Format listings are static pages generated by a cronjob and currently only support api version filtering
	public static function globalFormatPageFilterText()
	{
		$filters =[];
        $api_version = self::getGlobalApiVersion();
        if ($api_version && $api_version !== 'all') {
        ...
        if (count($filters) > 0) {
            echo "<div class=\"page-filter\"><i class=\"fas fa-filter\"></i> <a href=\"settings.php\">Global filters are applied:</a> " . implode(', ', $filters)."<div><b>Note:</b> Format listings currently only support filtering by api version, other active filters are omitted for this view.</div></div>";
        }
	}
}
